...
Tables~\ref{tab:solutionComparisonProperties1} and~\ref{tab:solutionComparisonProperties2} show how the solutions compare against each other when framed against some key properties. The majority of solutions use hardware to implement their schemes, and the single one which does not need additional hardware (C-FLAT \cite{Abera2016}) makes use of additional security functionality provided by ARM TrustZone. 
