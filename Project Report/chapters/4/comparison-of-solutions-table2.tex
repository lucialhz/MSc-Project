...
\midrule
C-FLAT \cite{Abera2016} & \LEFTcircle \raggedright \dag  & \CIRCLE & \raggedright Control flow instructions re-written to go via trampolines & Basic Block & \raggedright\arraybackslash TPM and Digital Signatures (for report)\\
...
\end{tabular}
\begin{tablenotes}
	\item \CIRCLE ~= has property; \LEFTcircle ~= partially has property; \Circle ~= does not have property; - = not enough information; \dag = makes use of hardware features not present in all embedded systems
\end{tablenotes}
\end{threeparttable}}
\end{table}